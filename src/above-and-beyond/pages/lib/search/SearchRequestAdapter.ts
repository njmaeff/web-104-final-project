export class SearchRequestAdapter {
    _adaptFacetFilters(facetFilters) {
        ...
        /**
         * Need to transform:
         *  facetFilters = [["field1:value1", "field1:value2"], "field2:value3",
         * "field2:value4"]
         *
         * Into this:
        ...
        const transformedTypesenseFilters = facetFilters.map((item) => {
            if (Array.isArray(item)) {
                // Need to transform:
                // facetFilters = ["field1:value1", "field1:value2",
                // "facetN:valueN"]  Into this: intermediateFacetFilters = {
                // "field1": ["value1", "value2"], "fieldN": ["valueN"] }

                const intermediateFacetFilters = {};
                item.forEach((facetFilter) => {
                    const facetFilterMatches = facetFilter.match(FILTER_STRING_MATCHING_REGEX);
                ...
                const facetFilterMatches = item.match(FILTER_STRING_MATCHING_REGEX);
                const fieldName = `${facetFilterMatches[1]}${facetFilterMatches[2]}`;
                const fieldValue = `${facetFilterMatches[3]}`;
    ...
    _isNumber(value) {
        return (
            Number.isInteger(value % 1) || // Mod 1 will automatically try converting
            // string values to integer/float
            !!(value % 1)
        ); // Is Float
    }
    ...
    _adaptIndexName(indexName) {
        return indexName.match(INDEX_NAME_MATCHING_REGEX)[1];
    }

    _adaptSortBy(indexName) {
        return indexName.match(INDEX_NAME_MATCHING_REGEX)[3];
    }
    ...
    constructor(public instantsearchRequests, public typesenseClient, public configuration) {
    ...
    collectionSpecificSearchParameters: any;
}

const INDEX_NAME_MATCHING_REGEX = new RegExp("^(.+?)(?=(/sort/(.*))|$)");

const FILTER_STRING_MATCHING_REGEX = new RegExp("(.*)((?!:).):(?!:)(.*)");
